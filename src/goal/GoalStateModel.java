...
public class GoalStateModel implements FullStateModel {
	...
	public GoalStateModel(double prob, int[] worldSize, int[] goalLoc) {
	...
	public State sample(State s, Action a) {
		GoalState gs = (GoalState) s.copy();
		int cur_dx = gs.goal_dx;
		int cur_dy = gs.goal_dy;
	...
	public List<StateTransitionProb> stateTransitions(State s, Action a) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (newPos[0] != cur_dx || newPos[1] != cur_dy) {
				...
				GoalState ns = (GoalState) gs.copy();
				ns.goal_dx = newPos[0];
				ns.goal_dy = newPos[1];
	...
	protected int[] moveResult(int cur_dx, int cur_dy, int direction) {
		...
		if (n_dx < 0 - this.goalLoc[0] || n_dx >= this.worldSize[0] - this.goalLoc[0] || n_dy < 0 - this.goalLoc[1]
				|| n_dy >= this.worldSize[1] - this.goalLoc[1]) {
			n_dx = cur_dx;
			n_dy = cur_dy;
		}
